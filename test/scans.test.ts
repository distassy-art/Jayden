...
test("field 12 Lotto payout is the Lotto/Scratchers scan, not Daily Book or Safe drop", () => {
  for (const day of ["2026-09-12", "2026-09-13"] as const) {
    const lotto = scanHref("hb", day, 12);
    const lottery = scanHref("hb", day, 13);
    const drop = scanHref("hb", day, 4);
    assert.equal(lotto, LOTTO_SCRATCHERS);
    assert.match(lotto ?? "", /115354/);
    assert.doesNotMatch(lotto ?? "", /lottery|safedrop|09122026|09132026/i);
    assert.notEqual(lotto, lottery);
    assert.notEqual(lotto, drop);
    assert.ok(lottery && lottery !== LOTTO_SCRATCHERS);
  }
});
