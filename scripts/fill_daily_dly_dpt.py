...
Usage:
  ...
  python3 scripts/fill_daily_dly_dpt.py --stage-dir /tmp/s2k/exports/stage \
      --through 2026-09-19   # S2K pull only; stage PDFs for Graph/MCP upload
"""
...
# When set (via --stage-dir), SharePoint uploads write here instead of live OD.
# Use when FedAuth digest is 403; upload staged PDFs via Graph/Onedrive MCP.
STAGE_DIR: Path | None = None
STAGE_UPLOADS: list[dict] = []
...
def _clients_rel(folder_rel: str) -> str:
    """Map SharePoint server-relative folder to drive path Clients/..."""
    marker = "/Documents/"
    if marker in folder_rel:
        return folder_rel.split(marker, 1)[1]
    if folder_rel.startswith("Clients/"):
        return folder_rel
    return f"Clients/{folder_rel.lstrip('/')}"
...
class SharePoint:
    def __init__(self, cookie_hdr: str):
        ...
        self.digest = "SOFT" if STAGE_DIR is not None else self._digest()
    ...
    def list_files(self, rel: str) -> list[dict]:
        if STAGE_DIR is not None:
            # Staging mode cannot list/recycle live OD; skip "exists" short-circuit
            # and older-MTD deletes (upload keep-latest via MCP separately).
            return []
        url = (
            BASE
    ...
    def upload(self, folder_rel: str, name: str, content: bytes) -> tuple[bool, str]:
        if STAGE_DIR is not None:
            dest_dir = STAGE_DIR / folder_rel.replace("/", "__")
            dest_dir.mkdir(parents=True, exist_ok=True)
            path = dest_dir / name
            path.write_bytes(content)
            od_path = f"{_clients_rel(folder_rel)}/{name}"
            STAGE_UPLOADS.append(
                {
                    "folder": folder_rel,
                    "name": name,
                    "path": str(path),
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Stage uploads under this directory instead of SharePoint "
        "(FedAuth-bypass). Writes upload_manifest.json with od_path entries "
        "for Graph / Onedrive MCP create_or_update_file.",
    )
    args = p.parse_args(argv)

    global STAGE_DIR, STAGE_UPLOADS
    if args.stage_dir:
    ...
    if STAGE_DIR is not None:
        manifest = {
            ...
            "mode": args.mode,
            "uploads": STAGE_UPLOADS,
        }
        man_path = STAGE_DIR / "upload_manifest.json"
        man_path.write_text(json.dumps(manifest, indent=2))
        print(f"Staged {len(STAGE_UPLOADS)} files -> {man_path}", flush=True)
        out["staged"] = len(STAGE_UPLOADS)
        out["stage_manifest"] = str(man_path)
