...
/** Cron fires at both PDT and PST UTC hours; the handler checks Pacific time. */
export const S2K_CRONS = ["0 17 * * *", "0 18 * * *", "0 19 * * *", "0 20 * * *"] as const;
 ...
 * Monday 10:00 → no pull (weekend waits for noon).
...
export function pullPlan(at: Date | number): PullPlan {
  ...
  if (!isOnHour(clock, PULL_HOUR_WEEKDAY) && !isOnHour(clock, PULL_HOUR_MONDAY)) {
    return skip(base, "not_10_or_noon");
  }
  const dow = weekdayIndex(clock.weekday);
  if (dow < 0) return skip(base, "unknown_weekday");

  if (isOnHour(clock, PULL_HOUR_WEEKDAY)) {
  ...
  // Monday noon only. Other weekdays at 12:00 do not pull.
  if (dow !== 1) return skip(base, "noon_not_monday");
  ...
  return guarded(base, "weekend", [friday, saturday, sunday], "monday_noon_fri_sat_sun");
}

function isOnHour(clock: PacificClock, hour: number): boolean {
  return clock.hour === hour && clock.minute < 5;
}
