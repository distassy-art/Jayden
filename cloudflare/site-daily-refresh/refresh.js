...
var SS_BILLING_VIEW_HTML = [
  '<style id="ss-billing-simple-v1">',
  ".bill-easy-kpis{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:0 0 14px}",
  ".bill-easy-kpis .val{font-size:1.5rem}",
  ".bill-easy-lead{margin:0 0 10px}",
  ".bill-easy-situation{margin:0 0 14px;font-size:15px;line-height:1.45;color:var(--navy,#0b1f33)}",
  ".bill-easy-h{margin:0 0 8px;font-size:1.02rem;color:var(--navy,#0b1f33);font-weight:750}",
  ".bill-easy-table-wrap{overflow:auto;border:1px solid var(--line,#d7e1ea);border-radius:12px;background:#fff}",
  ".bill-easy-table{width:100%;border-collapse:collapse;font-size:14px}",
  ".bill-easy-table th{text-align:left;font-size:11px;letter-spacing:.04em;text-transform:uppercase;color:var(--muted,#6b7c8f);padding:10px 12px;border-bottom:1px solid var(--line,#d7e1ea);background:#fafcfe}",
  ".bill-easy-table td{padding:11px 12px;border-bottom:1px solid #eef2f6}",
  ".bill-easy-table th.num,.bill-easy-table td.num{text-align:right;font-variant-numeric:tabular-nums}",
  ".bill-easy-table tr[data-bill-client]{cursor:pointer}",
  ".bill-easy-table tr[data-bill-client]:hover td{background:#f4f8fc}",
  ".bill-easy-owe{color:#9a5b00;font-weight:750}",
  ".bill-easy-ok{color:#0d6b35;font-weight:750}",
  ".bill-easy-sentence{font-size:1.05rem;line-height:1.5;color:var(--navy,#0b1f33);margin:0 0 12px}",
  ".bill-easy-back{margin:0 0 12px}",
  ...
  '<p class="lead bill-easy-lead" id="billEasyLead">Pick a month. All clients show what was billed, what is still owed, and what is paid. Open a client for that bill only.</p>',
  '<div class="bill-month-tabs" id="billMonthTabs" role="tablist" aria-label="Billing months"></div>',
  '<div class="bill-easy-kpis" id="billEasyKpis">',
  '<div class="cc-kpi"><span class="lbl">Billed</span><span class="val" id="billEasyBilled">—</span><span class="sub" id="billEasyBilledSub">This month</span></div>',
  '<div class="cc-kpi"><span class="lbl">Still owed</span><span class="val" id="billEasyOwed">—</span><span class="sub" id="billEasyOwedSub">Unpaid invoices</span></div>',
  ...
  '<div id="billAllWrap">',
  '<h3 class="bill-easy-h">All clients</h3>',
  '<div class="bill-easy-table-wrap" id="billClientTable"><p class="fine">Loading…</p></div>',
  '<p class="hint" id="billOwnerHint">Click a client to see the charges, the invoices, and the statement.</p>',
  '<div class="btn-row" id="billStatementBar" style="margin:10px 0 0">',
  '<button type="button" class="btn ghost" id="billStmtAllBtn" style="width:auto;padding:6px 12px;min-height:0">Open all statements</button>',
  '<span class="meta" id="billStmtHint"></span>',
  "</div>",
  "</div>",
  '<div id="billClientWrap" hidden>',
  '<button type="button" class="btn ghost bill-easy-back" id="billBackAll">All clients</button>',
  '<h3 class="bill-easy-h" id="billClientTitle">Client</h3>',
  '<p class="bill-easy-sentence" id="billClientSentence"></p>',
  '<h3 class="bill-easy-h">Charges still owed</h3>',
  '<div class="table-wrap"><table class="data bill-table"><thead><tr><th>Charge</th><th>Basis</th><th class="num">Qty / amount</th><th class="num">Amount</th></tr></thead><tbody id="billLinesBody"></tbody></table></div>',
  '<div id="billOwnerMonths" style="margin-top:14px"></div>',
  "</div>",
  '<div id="billStmtPanel" class="card panel bill-stmt-panel" style="margin:14px 0;display:none" aria-live="polite"></div>',
  '<div id="billOwnerDetail" class="card panel" style="margin-top:14px;display:none"></div>',
  '<p class="hint" id="billStripeFeeNote" data-stripe-fee-note="1">Payment processing (Stripe): ACH / e-check 0.8% (cap $5). Debit card or credit card 2.9% + $0.30.</p>',
  '<p class="hint" id="billPaidHint" hidden></p>',
  '<span id="billLinesMeta" hidden></span>',
  '<span id="billInvMeta" hidden></span>',
  '<div id="billOutBanner" hidden><strong id="billOutAmt"></strong><span id="billOutMeta"></span></div>',
  '<div id="billStmtList" hidden></div>'
].join("");

var SS_BILLING_OWNER_JS = [
  "        setText('billOwnerTitle', 'Billing');",
  "        setText('billOwnerMeta', (selectedMonth ? monthTitleSafe(selectedMonth) : 'Billing') + ' · ' + scope);",
  "        renderMonthTabs('billMonthTabs');",
  "        function billClientTotals(list) {",
  "          var billed = 0, owed = 0, paid = 0;",
  "          (list || []).forEach(function (inv) {",
  "            var tot = Number(inv.total) || 0;",
  "            billed += tot;",
  "            if (billIsPaid(inv)) paid += tot;",
  "            else owed += tot;",
  "          });",
  "          return { billed: billed, owed: owed, paid: paid };",
  "        }",
  "        function paintBillCards(tot, billedSub, owedSub, paidSub) {",
  "          setText('billEasyBilled', billMoney(tot.billed));",
  "          setText('billEasyOwed', billMoney(tot.owed));",
  "          setText('billEasyPaid', billMoney(tot.paid));",
  "          setText('billEasyBilledSub', billedSub);",
  "          setText('billEasyOwedSub', owedSub);",
  "          setText('billEasyPaidSub', paidSub);",
  "          var owedEl = document.getElementById('billEasyOwed');",
  "          if (owedEl) owedEl.className = 'val ' + (tot.owed > 0.004 ? 'bill-easy-owe' : 'bill-easy-ok');",
  "        }",
  "        var groups = groupByClient(monthInvs);",
  "        var monthName = selectedMonth ? monthTitleSafe(selectedMonth) : 'this month';",
  "        var single = typeof billIsSingleStation === 'function' && billIsSingleStation();",
  "        var selectedName = null;",
  "        if (single && groups.length === 1) selectedName = groups[0].client;",
  "        else if (window._billSelectedClient && groups.some(function (g) { return g.client === window._billSelectedClient; })) selectedName = window._billSelectedClient;",
  "        var allWrap = document.getElementById('billAllWrap');",
  "        var clientWrap = document.getElementById('billClientWrap');",
  "        var situation = document.getElementById('billEasySituation');",
  "        var chosen = null;",
  "        groups.forEach(function (g) { if (g.client === selectedName) chosen = g; });",
  "        if (selectedName && !chosen) selectedName = null;",
  "        if (!selectedName) {",
  "          var allTot = billClientTotals(monthInvs);",
  "          paintBillCards(allTot, groups.length + ' clients', allTot.owed > 0.004 ? 'Unpaid' : 'Nothing owed', 'Marked paid');",
  "          if (situation) situation.textContent = groups.length + ' clients in ' + monthName + '. Billed ' + billMoney(allTot.billed) + '. Still owed ' + billMoney(allTot.owed) + '. Paid ' + billMoney(allTot.paid) + '.';",
  "          if (allWrap) allWrap.hidden = false;",
  "          if (clientWrap) clientWrap.hidden = true;",
  "          var tableHost = document.getElementById('billClientTable');",
  "          if (tableHost) {",
  "            if (!groups.length) tableHost.innerHTML = '<p class=\"fine\">No invoices for this month.</p>';",
  "            else {",
  "              var body = groups.map(function (g) {",
  "                var t = billClientTotals(g.invoices);",
  "                var oweCls = t.owed > 0.004 ? ' bill-easy-owe' : ' bill-easy-ok';",
  "                return '<tr data-bill-client=\"' + tcEsc(g.client) + '\">' +",
  "                  '<td><strong>' + tcEsc(g.client) + '</strong></td>' +",
  "                  '<td class=\"num\">' + billMoney(t.billed) + '</td>' +",
  "                  '<td class=\"num' + oweCls + '\">' + billMoney(t.owed) + '</td>' +",
  "                  '<td class=\"num\">' + billMoney(t.paid) + '</td></tr>';",
  "              }).join('');",
  "              tableHost.innerHTML = '<table class=\"bill-easy-table\" aria-label=\"All clients billing\"><thead><tr>' +",
  "                '<th>Client</th><th class=\"num\">Billed</th><th class=\"num\">Still owed</th><th class=\"num\">Paid</th>' +",
  "                '</tr></thead><tbody>' + body + '</tbody></table>';",
  "              tableHost.querySelectorAll('[data-bill-client]').forEach(function (tr) {",
  "                tr.addEventListener('click', function () {",
  "                  window._billSelectedClient = tr.getAttribute('data-bill-client');",
  "                  buildBilling();",
  "                  try { var view = document.getElementById('view-billing'); if (view) view.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) {}",
  "                });",
  "              });",
  "            }",
  "          }",
  "          var allBtn = document.getElementById('billStmtAllBtn');",
  "          var stmtHint = document.getElementById('billStmtHint');",
  "          if (stmtHint) stmtHint.textContent = groups.length ? (groups.length + ' statements for ' + monthName) : '';",
  "          if (allBtn) {",
  "            allBtn.style.display = groups.length ? '' : 'none';",
  "            allBtn.onclick = function () {",
  "              var m = selectedMonth;",
  "              if (!m) return;",
  "              var withCharges = groups.filter(function (g) {",
  "                return g.invoices.some(function (inv) { return (Number(inv.total) || 0) > 0 || (Array.isArray(inv.lines) && inv.lines.length); });",
  "              });",
  "              if (!withCharges.length) { alert('No client charges for that month.'); return; }",
  "              if (withCharges.length === 1) { viewClientStatement(withCharges[0].client, m, withCharges[0].invoices); return; }",
  "              withCharges.forEach(function (g, i) { setTimeout(function () { openClientStatement(g.client, m, g.invoices); }, i * 250); });",
  "            };",
  "          }",
  "        } else {",
  "          var one = billClientTotals(chosen.invoices);",
  "          paintBillCards(one, chosen.client, one.owed > 0.004 ? 'Unpaid' : 'Paid up', monthName);",
  "          if (situation) situation.textContent = '';",
  "          var sentence = document.getElementById('billClientSentence');",
  "          if (sentence) sentence.textContent = chosen.client + ' in ' + monthName + '. Billed ' + billMoney(one.billed) + '. Still owed ' + billMoney(one.owed) + '. Paid ' + billMoney(one.paid) + '.';",
  "          var title = document.getElementById('billClientTitle');",
  "          if (title) title.textContent = chosen.client;",
  "          if (allWrap) allWrap.hidden = true;",
  "          if (clientWrap) clientWrap.hidden = false;",
  "          var back = document.getElementById('billBackAll');",
  "          if (back) {",
  "            back.hidden = !!single;",
  "            back.onclick = function () { window._billSelectedClient = null; buildBilling(); };",
  "          }",
  "          var clientAgg = billAggregateLines(chosen.invoices);",
  "          var linesBody = document.getElementById('billLinesBody');",
  "          if (linesBody) {",
  "            if (!clientAgg.rows.length) linesBody.innerHTML = '<tr><td colspan=\"4\">Nothing still owed on this client.</td></tr>';",
  "            else {",
  "              var lineSum = clientAgg.rows.reduce(function (a, r) { return a + (Number(r.fee) || 0); }, 0);",
  "              linesBody.innerHTML = clientAgg.rows.map(function (r) {",
  "                return '<tr><td><b>' + tcEsc(r.name) + '</b></td><td>' + tcEsc(r.basis) + '</td><td class=\"num\">' + tcEsc(billQtyLabel(r)) + '</td><td class=\"num\">' + billMoney(r.fee) + '</td></tr>';",
  "              }).join('') + '<tr class=\"bill-total\"><td colspan=\"3\">Still owed</td><td class=\"num\">' + billMoney(lineSum) + '</td></tr>';",
  "            }",
  "          }",
  "          var host = document.getElementById('billOwnerMonths');",
  "          if (host) {",
  ...
  "          }",
  "        }",
  "        if (role === 'admin') buildServicePricing();",
  ""
].join("\n");

function ssSimplifyBilling(html) {
  if (!html || html.indexOf("ss-billing-simple-v1") >= 0) return html;
  var ownerStart = html.indexOf('<div id="billingOwner">');
  var pricingStart = html.indexOf('<div id="billingPricing"');
  var jsNeedle = "        setText('billOwnerTitle', role === 'admin' ? 'Billing by month' : 'Invoice preview');";
  var jsEndMark = "        if (role === 'admin') buildServicePricing();";
  var jsStart = html.indexOf(jsNeedle);
  var jsEnd = jsStart < 0 ? -1 : html.indexOf(jsEndMark, jsStart);
  if (ownerStart < 0 || pricingStart < ownerStart || jsStart < 0 || jsEnd < 0) return html;
  var ownerOpen = '<div id="billingOwner">';
  html = html.slice(0, ownerStart + ownerOpen.length) + "\n" + SS_BILLING_VIEW_HTML + "\n      " + html.slice(pricingStart);
  jsStart = html.indexOf(jsNeedle);
  jsEnd = jsStart < 0 ? -1 : html.indexOf(jsEndMark, jsStart);
  if (jsStart < 0 || jsEnd < 0) return html;
