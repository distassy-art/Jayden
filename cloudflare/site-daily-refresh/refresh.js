...
var SS_BILLING_VIEW_HTML = [
  ...
  '<select id="billClientPick" aria-label="Billing client"><option value="">Choose a client</option></select>',
  "</div>",
...
var SS_BILLING_OWNER_JS = [
  ...
  "        var chooseHint = document.getElementById('billOwnerHint');",
  "        var chosen = null;",
  "        groups.forEach(function (g) { if (g.client === selectedName) chosen = g; });",
  "        if (selectedName && !chosen) selectedName = null;",
  "        var picker = document.getElementById('billClientPick');",
  "        if (picker) {",
  "          picker.innerHTML = '<option value=\"\">Choose a client</option>' + groups.map(function (g) {",
  "            return '<option value=\"' + tcEsc(g.client) + '\">' + tcEsc(g.client) + '</option>';",
  "          }).join('');",
  "          picker.value = selectedName || '';",
  "          picker.onchange = function () {",
  "            window._billSelectedClient = picker.value || null;",
  "            var stmt = document.getElementById('billStmtPanel');",
  "            if (stmt) { stmt.style.display = 'none'; stmt.innerHTML = ''; }",
  "            buildBilling();",
  "          };",
  "        }",
  "        if (!selectedName) {",
  "          var allTot = billClientTotals(monthInvs);",
  "          paintBillCards(allTot, monthName, allTot.owed > 0.004 ? 'Unpaid' : 'Nothing owed', 'Marked paid');",
  "          if (situation) situation.textContent = monthName + '. Billed ' + billMoney(allTot.billed) + '. Still owed ' + billMoney(allTot.owed) + '. Paid ' + billMoney(allTot.paid) + '.';",
  "          if (chooseHint) chooseHint.hidden = false;",
  "          if (clientWrap) clientWrap.hidden = true;",
  "        } else {",
  ...
  "          if (chooseHint) chooseHint.hidden = true;",
  "          if (clientWrap) clientWrap.hidden = false;",
